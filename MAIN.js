...
function Install() {
  ...
  if (SHOULD_APPLY_RETENTION) {
    _setDailyTrigger("NightlyRetentionProcess",1);
...
function processInbox() {
  /**
   * if we hit a rate limit, it will die somewhere else in thi script
  ...
  try {
    ...
    for (var i=0; i<threadlist.length; i++) {
      ...
      for (var j=0; j<convo.length; j++) {  
        ...
        //these things are undefined for chat logs, so will cause the script to error out
        var subject=email.getSubject();
        var to=email.getHeader("To");
        var from=email.getHeader("From");
        var recipients=to+" "+from

        var rulesMatched=0;

        if ( MatchFilters(StandardMessageFilters, thread, email) ) {
          //this removes retention policies for account emails
          rulesMatched++;
          Logger.log("[Standard]='"+subject.substring(0,15)+"...'");
        }
        ...
        if (SHOULD_PROCESS_MAILING_LISTS) {
          if ( MatchMailingLists(thread, email) ) {
            rulesMatched++;
            Logger.log("[Mailing List]='"+subject.substring(0,15)+"...'");
          }
        }

        if (SHOULD_PROCESS_AUTOMATED){
          if ( MatchAutomatedEmails(AutomatedMessageFilters, thread, email) ) {
            rulesMatched++;
            Logger.log("[Automated]='"+subject.substring(0,15)+"...'");
          }
        }

        if (SHOULD_PROCESS_SALESFORCE) {
          if ( MatchSalesforceFilters(SalesforceMessageFilters, thread, email) ) {
            rulesMatched++;
            Logger.log("[Salesforce]='"+subject.substring(0,15)+"...'");
          }
        }
        ...
        if (SHOULD_RENAME_TAGS) {
          if ( RenameTags(TagsToRename, thread, email) ) {
            rulesMatched++;
            Logger.log("[Renamed]='"+subject.substring(0,15)+"...'");
          }
        }
 ...
 * The trigger for kicking off each run remains the same.
 */
function NightlyRetentionProcess() {
  if (_getTriggerList("NightlyRetentionProcess").length == 0) {
    _setDailyTrigger("NightlyRetentionProcess",1);
  } else {
    // the only way to verify the correct schedule is to delete the triggers and start again
    _removeHandlerTriggers("NightlyRetentionProcess");
    _setDailyTrigger("NightlyRetentionProcess",1);
  }
